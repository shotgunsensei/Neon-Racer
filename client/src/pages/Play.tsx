...
export default function Play() {
  ...
  return (
    <main className="relative flex min-h-[100dvh] flex-col items-center overflow-hidden bg-background px-3 py-4 sm:px-6 sm:py-6">
      ...
      <div className="relative z-10 flex w-full max-w-5xl flex-1 flex-col gap-4">
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><p className="font-mono text-[10px] uppercase tracking-[.25em] text-secondary">Choose your exposure</p><h1 className="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">Enter the mainframe</h1></div>
          <p className="max-w-xs text-left text-sm text-muted-foreground sm:text-right">F triggers Neon Surge. In Chaos, Data Storm challenge waves escalate the run.</p>
        </div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3" role="radiogroup" aria-label="Game mode">
          {MODES.map((option) => {
            ...
            return <button key={option.value} role="radio" aria-checked={active} onClick={() => selectMode(option.value)} className={`group relative overflow-hidden border p-4 text-left transition-all focus:outline-none focus:ring-2 focus:ring-primary ${active ? "border-primary bg-primary/10 box-glow-primary" : "border-border/70 bg-card/60 hover:border-primary/50"}`}>
              ...
              <div className="flex items-center justify-between"><span className="font-display text-base font-bold uppercase tracking-wider">{option.label}</span><span className={`font-mono text-[9px] uppercase tracking-wider ${active ? "text-primary" : "text-muted-foreground"}`}>{option.stat}</span></div>
              <p className="mt-1 text-sm text-muted-foreground">{option.detail}</p>
            </button>;
          })}
        </div>
        <div className="flex flex-1 items-center justify-center py-1"><GameCanvas mode={selectedMode} onGameOver={handleGameOver} /></div>
        ...
        <div className="flex items-center justify-center gap-5 pb-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span className="inline-flex items-center gap-1"><Crosshair className="h-3 w-3 text-primary" /> ← → steer</span><span className="inline-flex items-center gap-1"><Zap className="h-3 w-3 text-accent" /> F surge</span></div>
      </div>
    </main>
  );
}